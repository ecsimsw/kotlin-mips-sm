...
class Memory(
    ...
    fun loadFile(path: String, address: Int) {
        ...
        DataInputStream(FileInputStream(path)).use {
            var saveIndex = address
            while (it.read(bytes) > 0) {
                saveToMemory(bytes, saveIndex++)
            }
        }
    }

    private fun saveToMemory(bytes: ByteArray, saveIndex: Int) {
        for (i in bytes.indices step (4)) {
            val binaryInstruction = bytes[i].toBinary(8) +
                    bytes[i + 1].toBinary(8) +
                    bytes[i + 2].toBinary(8) +
                    bytes[i + 3].toBinary(8)
            memory[saveIndex] = binaryInstruction.toLong(2).toInt()
        }
    }
